...
'When classification is requested, prefer one broader low-confidence LC class over an empty classification whenever the supplied evidence supports a defensible disciplinary or topical inference.',
'Return only the LC class portion (for example QA76.73), without a cutter, year, prefix, caption, prose, range, or terminal punctuation; the application builds the local call number separately.',
...
sub _task_instructions {
    ...
    return 'Suggest exactly one best defensible LCC class candidate whenever the title, creator, summary, notes, edition, or format provides any disciplinary or topical clue. Prefer a broader low-confidence class over an empty result. Return the class portion only, without cutter, year, prefix, caption, or prose. Distinguish explicit evidence from inference and calibrate confidence and specificity. Missing MARC fields are not evidence against a suggestion. Never return a range, terminal punctuation, schedule citation, or claim of verification. Use insufficient_evidence only when no LC class can be responsibly inferred at any useful level. Deterministic LCCS verification is performed separately by the application.'
