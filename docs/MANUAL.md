...
Open **ISBD Training** from the cataloguing toolbar. New trainees complete four onboarding screens before seeing the dashboard. Use **Continue training** to open the recommended lesson, or choose an unlocked module from the learning path. Each of the eleven modules has four lessons: a core lesson, two focused lessons, and a capstone. The complete path contains 44 lessons and 112 scored exercises. Complete lessons in order; mastering the complete module unlocks the next module.

Lessons combine short explanations, annotated MARC examples, interactive practice, progressive hints, feedback, and reflection. Choice positions are reshuffled on every display. In the MARC lab, edit the tag, indicators, subfield codes, values, and order, then choose **Check answer**. Exercise drafts and written reflections save automatically. **Show answer** is explicit and records the attempt as revealed, so it cannot award mastery. **Reset** starts a fresh independent attempt.
...
The curriculum is stored in `Koha/Plugin/Cataloging/AutoPunctuation/rules/intern_guide_v2.json` using schema `4.3.0`. Progress records course, guide, and rules versions. A version change preserves completed work and marks it for review.
