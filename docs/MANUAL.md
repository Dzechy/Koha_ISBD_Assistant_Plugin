...
Open **ISBD Training** from the cataloguing toolbar. New trainees complete four onboarding screens before seeing the dashboard. Use **Continue training** to open the recommended lesson, or choose an unlocked module from the learning path. Each of the eleven modules has one comprehensive lesson with at least ten scored exercises arranged in non-decreasing difficulty. The complete path contains 112 exercises; mastering a lesson unlocks the next module.
...
The curriculum is stored in `Koha/Plugin/Cataloging/AutoPunctuation/rules/intern_guide_v2.json` using schema `4.2.0`. Progress records course, guide, and rules versions. A version change preserves completed work and marks it for review.
